namespace Scumbag {
  /**
   * Creates a generator function with some args.
   * @param code is the text of the code to create the generator.
   * @param env is a bunch of stuff to make available to the run code.
   * @returns the created generator as an iterator object.
   */
  ...
  /**
   * Saves an actor's position.
   * @param actor is a sprite which we check if is an actor then save if it is.
   */
  function storeActor(actor: Phaser.Sprite): void
  {
    if (!(actor instanceof Actor)) return;
    StateOfGame.parameters.actors.push({
      name: actor.name,
      x: actor.x,
      y: actor.y
    });
  ...
  /**
   * Stores the positions of all actors in the current state.
  ...
  export class Controller {
    script: Iterator<any>;
    states: Array<{minHealth: number,script: Iterator<any>}> =
      new Array<{minHealth: number, script: Iterator<any>}>();
    caller: any;
    game: Phaser.Game;
    ...
    constructor(game: Phaser.Game, scriptName: string, caller: any) {
      ...
      this.script = createGenerator(
        game.cache.getText(scriptName),
        {
          "state": <Overworld>(game.state.getCurrentState()),
          "caller": caller,
          "input": input,
          "Axis": Axis,
          "Button": Button,
