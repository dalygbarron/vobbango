...
function* legs()
{
  ...
  while (true)
  {
    ...
    for (var i = 0;i < LEG_LENGTH;i++)
    {
      for (leg of legList)
      {
        legBullets.fire((caller.body.x + caller.body.width / 2),(caller.y),0,0,leg);
      }
...
function* poison()
{
  ...
  while (true)
  {
    for (var i = 0;i < LEG_LENGTH;i++)
    {
      for (leg in legList)
      {
        legBullets.fire((caller.body.x + caller.body.width / 2),(caller.y),0,0,legList[leg]);
      }
      yield* wait(LEG_PERIOD);
    }
    poisonBullets.fire((caller.body.x + caller.body.width / 2),(caller.y),0,0,getAngleToPlayer());
    legList.shift();
...
function* burrow()
{
  ...
  while (true)
  {
    mud.fire((caller.body.x + caller.body.width / 2),(caller.y),0,10,Math.random() * Math.PI * 2 - Math.PI);
    yield* wait(17);
  }
}
